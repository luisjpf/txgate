...
//! - [`TxGateError`] - Top-level error that wraps all error types
//!
//! # Example
//!
//! ```rust
...
impl From<&TxGateError> for RpcErrorCode {
    fn from(error: &TxGateError) -> Self {
        match error {
            TxGateError::Parse(ParseError::UnsupportedChain { .. }) => Self::ChainNotSupported,
            TxGateError::Parse(_) => Self::ParseFailed,
            TxGateError::PolicyDenied { .. } | TxGateError::Policy(_) => Self::PolicyDenied,
            TxGateError::Sign(SignError::KeyNotFound { .. })
            | TxGateError::Store(StoreError::KeyNotFound { .. }) => Self::KeyNotFound,
            TxGateError::Sign(_) => Self::SignatureFailed,
            TxGateError::Store(_) | TxGateError::Config(_) => Self::InternalError,
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_txgate_error_from_parse_error() {
        let parse_err = ParseError::UnknownTxType;
        let txgate_err: TxGateError = parse_err.into();

        assert!(matches!(
            txgate_err,
            TxGateError::Parse(ParseError::UnknownTxType)
        ));
        assert_eq!(
            txgate_err.to_string(),
            "Parse error: unknown transaction type"
        );
    }

    #[test]
    fn test_txgate_error_from_sign_error() {
        let sign_err = SignError::key_not_found("test-key");
        let txgate_err: TxGateError = sign_err.into();
    ...
    #[test]
    fn test_txgate_error_from_store_error() {
        let store_err = StoreError::DecryptionFailed;
        let txgate_err: TxGateError = store_err.into();

        assert!(matches!(
            txgate_err,
            TxGateError::Store(StoreError::DecryptionFailed)
        ));
        assert_eq!(
            txgate_err.to_string(),
    ...
    #[test]
    fn test_txgate_error_from_policy_error() {
        let policy_err = PolicyError::blacklisted("0x1234");
        let txgate_err: TxGateError = policy_err.into();

        assert!(matches!(
            txgate_err,
            TxGateError::Policy(PolicyError::Blacklisted { .. })
        ));
        assert_eq!(
            txgate_err.to_string(),
    ...
    #[test]
    fn test_txgate_error_from_config_error() {
        let config_err = ConfigError::missing_field("api_key");
        let txgate_err: TxGateError = config_err.into();

        assert!(matches!(
            txgate_err,
            TxGateError::Config(ConfigError::MissingField { .. })
        ));
        assert_eq!(
            txgate_err.to_string(),
    ...
    #[test]
    fn test_txgate_error_policy_denied() {
        let err = TxGateError::policy_denied("whitelist", "address not in whitelist");

        assert!(matches!(err, TxGateError::PolicyDenied { .. }));
        assert_eq!(
            err.to_string(),
    ...
    #[test]
    fn test_rpc_error_code_from_txgate_error() {
        // Parse errors
        let err = TxGateError::Parse(ParseError::UnknownTxType);
        assert_eq!(RpcErrorCode::from(&err), RpcErrorCode::ParseFailed);

        let err = TxGateError::Parse(ParseError::unsupported_chain("unknown"));
        assert_eq!(RpcErrorCode::from(&err), RpcErrorCode::ChainNotSupported);

        // Sign errors
        let err = TxGateError::Sign(SignError::key_not_found("test"));
        assert_eq!(RpcErrorCode::from(&err), RpcErrorCode::KeyNotFound);

        let err = TxGateError::Sign(SignError::InvalidKey);
        assert_eq!(RpcErrorCode::from(&err), RpcErrorCode::SignatureFailed);

        // Policy errors
        let err = TxGateError::policy_denied("whitelist", "not allowed");
        assert_eq!(RpcErrorCode::from(&err), RpcErrorCode::PolicyDenied);

        let err = TxGateError::Policy(PolicyError::blacklisted("0x1234"));
        assert_eq!(RpcErrorCode::from(&err), RpcErrorCode::PolicyDenied);

        // Store errors
        let err = TxGateError::Store(StoreError::key_not_found("test"));
        assert_eq!(RpcErrorCode::from(&err), RpcErrorCode::KeyNotFound);

        let err = TxGateError::Store(StoreError::DecryptionFailed);
        assert_eq!(RpcErrorCode::from(&err), RpcErrorCode::InternalError);

        // Config errors
        let err = TxGateError::Config(ConfigError::missing_field("test"));
        assert_eq!(RpcErrorCode::from(&err), RpcErrorCode::InternalError);
    }
    ...
    #[test]
    fn test_txgate_error_is_send_sync() {
        fn assert_send_sync<T: Send + Sync>() {}
        assert_send_sync::<TxGateError>();
