//! # Ethereum Address Command
//!
//! Implementation of the `sello ethereum address` command that displays
//! the Ethereum address derived from the default signing key.
//!
//! ## Usage
//!
//! ```no_run
//! use sello::cli::commands::ethereum::AddressCommand;
//!
...
//! ## Output
//!
//! The command outputs the Ethereum address in EIP-55 checksummed format:
//!
//! ```text
...
/// Errors that can occur when displaying the Ethereum address.
...
/// The `sello ethereum address` command handler.
///
/// This command displays the Ethereum address derived from the default
/// signing key. The address is output in EIP-55 checksummed format.
///
/// # Example
///
/// ```no_run
...
impl AddressCommand {
    ...
    /// 5. Derives and displays the Ethereum address
    ///
    /// # Errors
    ///
    /// Returns an error if:
...
mod tests {
    ...
    #[test]
    fn test_address_is_eip55_checksummed() {
        let temp_dir = create_test_dir();
        let (base_dir, passphrase) = setup_initialized_env(&temp_dir);
        ...
        // EIP-55 addresses have mixed case (unless all digits)
        // At minimum, verify it's a valid hex address
        let hex_part = &address[2..];
        assert!(hex_part.chars().all(|c| c.is_ascii_hexdigit()));

        // Verify it's not all lowercase (which would indicate no checksum)
        // Note: Statistically, an EIP-55 address almost always has mixed case
        // unless the address happens to have no a-f characters
        let has_letters = hex_part.chars().any(|c| c.is_alphabetic());
        if has_letters {
            let has_upper = hex_part.chars().any(|c| c.is_ascii_uppercase());
            let has_lower = hex_part.chars().any(|c| c.is_ascii_lowercase());
            // Most addresses have mixed case due to EIP-55
            // This is probabilistic but very likely to pass
            assert!(
                has_upper || has_lower,
                "Address should have EIP-55 checksum formatting"
            );
        }
    }
