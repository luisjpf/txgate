...
//!
//! ```no_run
//! use sello::audit::{AuditLogger, SignEvent, PolicyResultInput};
//! use std::path::Path;
//!
//! // Initialize with a 32-byte HMAC key
//! let key = [0u8; 32]; // In production, use a secure random key
//! let logger = AuditLogger::new(Path::new("/var/log/sello"), &key)
...
impl AuditLogger {
    ...
    ///
    /// ```no_run
    /// use sello::audit::AuditLogger;
    /// use std::path::Path;
    ///
    /// let key = [0u8; 32]; // Use a secure random key in production
    /// let logger = AuditLogger::new(Path::new("/var/log/sello"), &key)?;
    /// # Ok::<(), sello::audit::AuditError>(())
    /// ```
    ...
    ///
    /// Reads the HMAC key from `~/.sello/audit.key` (or the specified base directory).
    /// The key file should contain exactly 32 bytes (or 64 hex characters).
    ///
    /// # Arguments
    ///
    /// * `base_dir` - Base directory containing configuration files (typically `~/.sello`)
    ///
    /// # Errors
    ///
    /// Returns [`AuditError`] if:
    ...
    /// use std::path::Path;
    ///
    /// let logger = AuditLogger::from_config(Path::new("/home/user/.sello"))?;
    /// # Ok::<(), sello::audit::AuditError>(())
    /// ```
