//! Configuration loader for the Sello signing service.
//!
...
//! # Default Location
//!
//! Configuration is stored at `~/.sello/config.toml` by default.
//!
//! # Examples
//!
...
//!
//! ```no_run
//! use sello_core::config_loader::load_config;
//!
...
//!
//! // Create loader with default base directory (~/.sello)
...
//! use std::path::PathBuf;
//!
//! let loader = ConfigLoader::with_base_dir(PathBuf::from("/custom/sello"));
//! let config = loader.load().expect("failed to load config");
//! ```
...
/// use sello_core::config_loader::ConfigLoader;
///
/// // Create with default base directory (~/.sello)
/// let loader = ConfigLoader::new().expect("failed to create loader");
///
...
pub struct ConfigLoader {
    /// Base directory for Sello files (default: ~/.sello).
    base_dir: PathBuf,
}

impl ConfigLoader {
    /// Creates a new `ConfigLoader` with the default base directory (`~/.sello`).
    ///
    /// # Errors
    ///
    ...
    /// # Arguments
    ///
    /// * `base_dir` - The base directory for Sello files
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// // path is something like "/home/user/.sello/config.toml"
    /// ```
    #[must_use]
    ...
    /// Returns the base directory for Sello files.
    ///
    /// # Examples
    ///
    /// ```no_run
    ...
    /// // base is something like "/home/user/.sello"
    /// ```
    #[must_use]
...
///
/// ```no_run
/// use sello_core::config_loader::expand_path;
///
/// // Expands ~ to home directory
/// let path = expand_path("~/.sello/config.toml").expect("failed to expand path");
/// // path is something like "/home/user/.sello/config.toml"
///
/// // Absolute paths are unchanged
/// let path = expand_path("/etc/sello/config.toml").expect("failed to expand path");
/// assert_eq!(path.to_string_lossy(), "/etc/sello/config.toml");
/// ```
...
/// Returns the default base directory for Sello files (`~/.sello`).
///
/// # Errors
///
/// Returns [`ConfigError::NoHomeDirectory`] if the home directory cannot be determined.
...
///
/// ```no_run
/// use sello_core::config_loader::default_base_dir;
///
/// let base_dir = default_base_dir().expect("failed to get base dir");
/// // base_dir is something like "/home/user/.sello"
/// ```
...
///
/// ```no_run
/// use sello_core::config_loader::load_config_with_expanded_paths;
///
/// let config = load_config_with_expanded_paths().expect("failed to load config");
/// // socket_path is something like "/home/user/.sello/sello.sock"
/// println!("Socket path: {}", config.server.socket_path);
/// ```
...
mod tests {
    ...
    fn test_expand_path_with_absolute_path() {
        let result = expand_path("/etc/sello/config.toml");
        assert!(result.is_ok());

        let path = result.expect("should succeed");
        assert_eq!(path, PathBuf::from("/etc/sello/config.toml"));
    }
    ...
    fn test_expand_path_with_relative_path() {
        ...
        let path = result.expect("should succeed");
        assert_eq!(path, PathBuf::from("sello/config.toml"));
    }
    ...
    fn test_default_base_dir() {
        ...
        let path = result.expect("should succeed");
        let home = dirs::home_dir().expect("home dir should exist");
        assert_eq!(path, home.join(".sello"));
    }
    ...
    fn test_config_loader_with_base_dir() {
        let custom_path = PathBuf::from("/custom/sello");
...
"#;
        ...
        // Default values
        assert_eq!(config.server.socket_path, "~/.sello/sello.sock");
        assert_eq!(config.keys.directory, "~/.sello/keys");
    ...
    fn test_save_creates_directory_and_file() {
        let temp_dir = TempDir::new().expect("failed to create temp dir");
        let base_dir = temp_dir.path().join("nested/sello");
    ...
    fn test_write_default_creates_valid_config() {
        ...
        let loaded = loader.load().expect("load should succeed");
        assert_eq!(loaded.server.socket_path, "~/.sello/sello.sock");
        assert_eq!(loaded.server.timeout_secs, 30);
        assert_eq!(loaded.keys.directory, "~/.sello/keys");
    ...
    fn test_write_default_creates_nested_directory() {
        let temp_dir = TempDir::new().expect("failed to create temp dir");
        let base_dir = temp_dir.path().join("deeply/nested/sello/dir");
        let loader = ConfigLoader::with_base_dir(base_dir.clone());
