//! Configuration types for the Sello signing service.
//!
//! This module provides the configuration structures for defining the behavior
//! of the Sello daemon, including server settings, key storage configuration,
//! and policy rules.
//!
//! # Configuration File
//!
//! Configuration is stored in TOML format at `~/.sello/config.toml`.
//!
//! # Examples
//!
//! ```
//! use sello_core::config::{Config, ServerConfig, KeysConfig};
//!
...
/// Top-level configuration for the Sello signing service.
///
/// This struct contains all configuration sections for the Sello daemon:
///
...
/// let toml_str = r#"
/// [server]
/// socket_path = "/var/run/sello.sock"
/// timeout_secs = 60
///
/// [keys]
/// directory = "/var/lib/sello/keys"
/// default_key = "main"
///
/// [policy]
...
pub struct Config {
    /// Server configuration for the Sello daemon.
    #[serde(default)]
...
/// Returns the default Unix socket path for the Sello daemon.
///
/// The default path is `~/.sello/sello.sock`.
#[must_use]
fn default_socket_path() -> String {
    "~/.sello/sello.sock".to_string()
}
...
/// Server configuration for the Sello daemon.
///
/// This struct defines how the Sello daemon listens for requests:
///
...
///
/// ```
/// use sello_core::config::ServerConfig;
///
/// let config = ServerConfig::default();
/// assert_eq!(config.socket_path, "~/.sello/sello.sock");
/// assert_eq!(config.timeout_secs, 30);
/// ```
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct ServerConfig {
    /// Unix socket path for the Sello daemon.
    ///
...
///
/// The default directory is `~/.sello/keys`.
#[must_use]
fn default_keys_dir() -> String {
    "~/.sello/keys".to_string()
}
...
/// Key storage configuration for the Sello signing service.
///
...
///
/// ```
/// use sello_core::config::KeysConfig;
///
/// let config = KeysConfig::default();
/// assert_eq!(config.directory, "~/.sello/keys");
...
///
/// ```
/// use sello_core::config::PolicyConfig;
///
...
"#
    ...
    ///     .build();
    ///
    /// assert_eq!(config.server.socket_path, "/var/run/sello.sock");
    /// ```
    #[must_use]
...
///
/// ```
/// use sello_core::config::{Config, ConfigBuilder};
///
...
mod tests {
    ...
    fn test_server_config_custom() {
        let config = ServerConfig {
            socket_path: "/var/run/sello.sock".to_string(),
            timeout_secs: 60,
        };

        assert_eq!(config.socket_path, "/var/run/sello.sock");
    ...
    fn test_keys_config_custom() {
        let config = KeysConfig {
            directory: "/var/lib/sello/keys".to_string(),
            default_key: "production".to_string(),
        };

        assert_eq!(config.directory, "/var/lib/sello/keys");
    ...
    fn test_toml_deserialization_with_defaults() {
        ...
        assert_eq!(config.keys.directory, "~/.sello/keys"); // default
    ...
    fn test_toml_deserialization_partial_sections() {
        ...
        assert_eq!(config.server.socket_path, "~/.sello/sello.sock"); // default
        assert_eq!(config.server.timeout_secs, 120);
        assert_eq!(config.keys.directory, "~/.sello/keys"); // default
    ...
    fn test_toml_deserialization_full_config() {
        ...
        assert_eq!(config.server.socket_path, "/var/run/sello.sock");
        assert_eq!(config.server.timeout_secs, 60);
        assert_eq!(config.keys.directory, "/var/lib/sello/keys");
        assert_eq!(config.keys.default_key, "production");
        assert!(config.policy.whitelist_enabled);
        assert_eq!(
    ...
    fn test_default_toml_contains_default_values() {
        ...
        assert!(toml.contains("socket_path = \"~/.sello/sello.sock\""));
        assert!(toml.contains("timeout_secs = 30"));
        assert!(toml.contains("directory = \"~/.sello/keys\""));
    ...
    fn test_builder_chain() {
        let config = Config::builder()
            .socket_path("/var/run/sello.sock")
            .timeout_secs(60)
            .keys_directory("/var/lib/sello/keys")
            .default_key("main")
            .build();
