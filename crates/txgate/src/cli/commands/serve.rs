//! # Serve Command
//!
//! Start the `TxGate` signing server as a long-running process.
//!
...
/// The key is loaded from `~/.txgate/keys/default.enc` and decrypted using
/// the provided passphrase.
///
/// # Example
///
/// ```no_run
/// use txgate::cli::commands::ServeCommand;
///
/// #[tokio::main]
/// async fn main() {
...
pub enum ServeError {
    /// `TxGate` has not been initialized. Run `txgate init` first.
...
/// Get the base `TxGate` directory (~/.txgate).
...
/// Check if `TxGate` has been initialized.
...
#[cfg(not(unix))]
fn warn_if_config_writable(_config_path: &Path) {
    // File permission checks are Unix-specific
}
...
fn load_key(base_dir: &Path, passphrase: &str) -> Result<Secp256k1KeyPair, ServeError> {
    ...
    let secret_key = store.load("default", passphrase).map_err(|e| {
        ...
        if matches!(e, txgate_core::error::StoreError::DecryptionFailed) {
...
mod tests {
    ...
    /// Helper to create a test environment with initialized `TxGate`.
    ...
    mod writability_warning_tests {
        use super::*;

        #[test]
        #[cfg(unix)]
        fn test_warn_if_config_writable_does_not_panic() {
            use std::os::unix::fs::PermissionsExt;
        ...
        #[test]
        fn test_warn_if_config_writable_nonexistent_file() {
            // Should not panic when file doesn't exist
            warn_if_config_writable(Path::new("/tmp/nonexistent-txgate-config.toml"));
        }
    }
