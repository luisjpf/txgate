//! # Config Command
//!
//! Implementation of the `txgate config` command that displays, edits, and
//! manages the `TxGate` configuration file.
//!
//! ## Usage
//!
//! ```text
//! txgate config           # Display current configuration
//! txgate config edit      # Open configuration in $EDITOR
//! txgate config path      # Show the configuration file path
//! ```
//!
//! ## Example
//!
//! ```no_run
//! use txgate::cli::commands::config::ConfigCommand;
//! use txgate::cli::args::ConfigAction;
//!
//! // Display config
...
pub enum ConfigCommandError {
    /// `TxGate` is not initialized (no config file exists).
...
/// The `txgate config` command handler.
///
/// This command handles viewing and editing the `TxGate` configuration file.
///
/// # Example
///
/// ```no_run
...
impl ConfigCommand {
    ...
    ///
    /// Returns an error if:
    /// - `TxGate` is not initialized (no config file)
...
mod tests {
    ...
    fn test_show_path_returns_correct_path() {
        ...
        let expected = home.join(".txgate").join("config.toml");
...
"#;
    ...
    fn test_get_base_dir() {
        ...
        let home = dirs::home_dir().expect("should get home dir");
        assert_eq!(base_dir, home.join(".txgate"));
    }
