//! # Status Command
//!
//! Implementation of the `txgate status` command that displays current status
...
//!
//! ```no_run
//! use txgate::cli::commands::status::StatusCommand;
//!
...
use txgate_core::config::{Config, PolicyConfig};
use txgate_core::config_loader::{expand_path, ConfigLoader};
...
/// The `txgate status` command handler.
///
/// This command displays current `TxGate` status including:
...
mod tests {
    ...
    /// Create an initialized `TxGate` directory structure.
    fn create_initialized_txgate(base_dir: &Path) {
        // Create directories
    ...
    fn test_is_initialized_true_when_complete() {
        ...
        create_initialized_txgate(base_dir);
...
"#;
    ...
    fn test_format_display_path_without_home() {
        let path = Path::new("/etc/txgate/config.toml");
        let display = format_display_path(path);
        assert_eq!(display, "/etc/txgate/config.toml");
    }
