//! # Init Command
//!
//! Implementation of the `txgate init` command that initializes the `TxGate`
...
use txgate_core::error::{ConfigError, StoreError};
...
pub enum InitError {
    /// `TxGate` is already initialized and --force was not specified.
    #[error("TxGate is already initialized. Use --force to reinitialize.")]
    AlreadyInitialized,
...
/// The `txgate init` command handler.
///
/// This command initializes the `TxGate` configuration directory structure,
...
impl InitCommand {
    ...
    /// 2. Creates ~/.txgate directory structure
...
/// Check if `TxGate` is already initialized.
///
...
/// Create the directory structure for `TxGate`.
...
fn print_success_message(eth_address: &str) {
    ...
    println!("  2. View status: txgate status");
    println!("  3. Start server: txgate serve");
}
...
mod tests {
    ...
    fn test_directory_permissions() {
        let temp_dir = create_test_dir();
        let base_dir = temp_dir.path().join("txgate-test");
    ...
    fn test_init_error_display() {
        assert_eq!(
            ...
            "TxGate is already initialized. Use --force to reinitialize."
        );
    ...
    fn test_directory_creation_is_idempotent() {
        let temp_dir = create_test_dir();
        let base_dir = temp_dir.path().join("txgate-idempotent-test");
