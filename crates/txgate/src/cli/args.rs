...
//!
//! - `txgate init` - Initialize `TxGate` configuration
...
//! - `txgate ethereum sign <TX_HEX>` - Sign an Ethereum transaction
//!
//! ## Global Options
//!
...
///
/// `TxGate` provides a secure environment for signing blockchain transactions
...
pub struct Cli {
    ...
    /// Path to config file
    ///
    /// If not specified, `TxGate` will look for configuration in the default
    /// location (`~/.config/txgate/config.toml` on Unix, or the appropriate
...
pub enum Commands {
    /// Initialize `TxGate` configuration and generate default key
    ///
...
impl From<CurveArg> for txgate_crypto::signer::CurveType {
...
mod tests {
    ...
    fn test_parse_init_force() {
        let cli = Cli::try_parse_from(["txgate", "init", "--force"]);
        assert!(
            cli.is_ok(),
    ...
    fn test_parse_status() {
        let cli = Cli::try_parse_from(["txgate", "status"]);
    ...
    fn test_parse_config() {
        let cli = Cli::try_parse_from(["txgate", "config"]);
    ...
    fn test_parse_config_edit() {
        let cli = Cli::try_parse_from(["txgate", "config", "edit"]);
        assert!(
            cli.is_ok(),
    ...
    fn test_parse_config_path() {
        let cli = Cli::try_parse_from(["txgate", "config", "path"]);
        assert!(
            cli.is_ok(),
    ...
    #[test]
    fn test_parse_serve() {
        let cli = Cli::try_parse_from(["txgate", "serve"]);
    ...
    fn test_parse_serve_foreground() {
        let cli = Cli::try_parse_from(["txgate", "serve", "--foreground"]);
        assert!(
            cli.is_ok(),
    ...
    fn test_parse_ethereum_address() {
        let cli = Cli::try_parse_from(["txgate", "ethereum", "address"]);
        assert!(
            cli.is_ok(),
    ...
    fn test_parse_ethereum_sign() {
        let cli = Cli::try_parse_from(["txgate", "ethereum", "sign", "0xdeadbeef"]);
        assert!(
            cli.is_ok(),
    ...
    fn test_parse_verbose_levels() {
        // Single -v
        let cli = Cli::try_parse_from(["txgate", "-v", "status"]);
        assert!(cli.is_ok(), "Failed to parse '-v': {:?}", cli.err());
        let cli = cli.expect("CLI should parse");
        assert_eq!(cli.verbose, 1);

        // Double -vv
        let cli = Cli::try_parse_from(["txgate", "-vv", "status"]);
        assert!(cli.is_ok(), "Failed to parse '-vv': {:?}", cli.err());
        let cli = cli.expect("CLI should parse");
        assert_eq!(cli.verbose, 2);

        // Triple -vvv
        let cli = Cli::try_parse_from(["txgate", "-vvv", "status"]);
    ...
    fn test_parse_config_option() {
        let cli = Cli::try_parse_from(["txgate", "--config", "/path/to/config.toml", "status"]);
        assert!(cli.is_ok(), "Failed to parse '--config': {:?}", cli.err());
        let cli = cli.expect("CLI should parse");
    ...
    fn test_parse_config_short_option() {
        let cli = Cli::try_parse_from(["txgate", "-c", "/path/to/config.toml", "status"]);
    ...
    fn test_global_options_with_subcommands() {
        ...
        let cli = Cli::try_parse_from(["txgate", "status", "-v"]);
        ...
        assert!(cli.is_ok());
        let cli = cli.expect("CLI should parse");
        assert_eq!(cli.config, Some(PathBuf::from("/etc/txgate.toml")));
    }
    ...
    fn test_invalid_command() {
        let cli = Cli::try_parse_from(["txgate", "invalid"]);
    ...
    fn test_missing_required_arg() {
        ...
        let cli = Cli::try_parse_from(["txgate", "ethereum", "sign"]);
    ...
    fn test_no_command_shows_help() {
        ...
        let cli = Cli::try_parse_from(["txgate"]);
    ...
    fn test_parse_key_list() {
        let cli = Cli::try_parse_from(["txgate", "key", "list"]);
    ...
    fn test_parse_key_list_details() {
        let cli = Cli::try_parse_from(["txgate", "key", "list", "--details"]);
        assert!(
            cli.is_ok(),
    ...
    fn test_parse_key_import() {
        let cli = Cli::try_parse_from(["txgate", "key", "import", "0xdeadbeef"]);
    ...
    fn test_parse_key_import_with_name() {
        let cli =
            Cli::try_parse_from(["txgate", "key", "import", "0xdeadbeef", "--name", "my-key"]);
        assert!(
            cli.is_ok(),
    ...
    fn test_parse_key_export() {
        let cli = Cli::try_parse_from(["txgate", "key", "export", "my-key"]);
    ...
    fn test_parse_key_delete() {
        let cli = Cli::try_parse_from(["txgate", "key", "delete", "my-key"]);
    ...
    fn test_parse_key_delete_with_force() {
        let cli = Cli::try_parse_from(["txgate", "key", "delete", "my-key", "--force"]);
        assert!(
            cli.is_ok(),
    ...
    fn test_key_requires_subcommand() {
        let cli = Cli::try_parse_from(["txgate", "key"]);
    ...
    fn test_curve_arg_to_curve_type_conversion() {
        use txgate_crypto::signer::CurveType;
