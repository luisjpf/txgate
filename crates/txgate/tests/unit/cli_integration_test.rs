...
use clap::Parser;
use txgate::cli::{Cli, Commands, ConfigAction, EthereumCommands, OutputFormat};
...
fn test_cli_command_dispatch() {
    ...
    // Test config command
    let cli = Cli::try_parse_from(["txgate", "config"]).expect("should parse");
    assert!(matches!(cli.command, Commands::Config { action: None }));

    // Test serve command
    let cli = Cli::try_parse_from(["txgate", "serve"]).expect("should parse");
...
fn test_ethereum_subcommand_dispatch() {
    ...
    let cli = Cli::try_parse_from(["txgate", "ethereum", "address"]).expect("should parse");
    match cli.command {
        Commands::Ethereum {
    ...
    let cli = Cli::try_parse_from(["txgate", "ethereum", "sign", "0xabcd", "--format", "hex"])
        .expect("should parse");
    match cli.command {
        Commands::Ethereum {
            command:
    ...
    let cli = Cli::try_parse_from(["txgate", "ethereum", "sign", "0x1234", "--format", "json"])
        .expect("should parse");
    match cli.command {
        Commands::Ethereum {
            command:
...
fn test_verbose_flag_parsing() {
    ...
    // Single verbose
    let cli = Cli::try_parse_from(["txgate", "-v", "status"]).expect("should parse");
    assert_eq!(cli.verbose, 1);

    // Double verbose
    let cli = Cli::try_parse_from(["txgate", "-vv", "status"]).expect("should parse");
    assert_eq!(cli.verbose, 2);

    // Triple verbose
    let cli = Cli::try_parse_from(["txgate", "-vvv", "status"]).expect("should parse");
    assert_eq!(cli.verbose, 3);

    // Verbose flag after command
    let cli = Cli::try_parse_from(["txgate", "status", "-v"]).expect("should parse");
    assert_eq!(cli.verbose, 1);
}
...
fn test_config_file_option() {
    ...
    // Short form
    let cli =
        Cli::try_parse_from(["txgate", "-c", "/etc/txgate.toml", "status"]).expect("should parse");
    assert_eq!(
        cli.config.as_deref().unwrap().to_str().unwrap(),
        "/etc/txgate.toml"
    );
...
fn test_config_action_parsing() {
    ...
    let cli = Cli::try_parse_from(["txgate", "config", "edit"]).expect("should parse");
    match cli.command {
        Commands::Config {
    ...
    let cli = Cli::try_parse_from(["txgate", "config", "path"]).expect("should parse");
    match cli.command {
        Commands::Config {
...
fn test_serve_command_options() {
    ...
    let cli = Cli::try_parse_from(["txgate", "serve", "--foreground"]).expect("should parse");
    match cli.command {
    ...
    let cli = Cli::try_parse_from(["txgate", "serve", "-f"]).expect("should parse");
    match cli.command {
        Commands::Serve { foreground: true } => {}
        _ => panic!("Expected Serve command with foreground"),
    }
...
fn test_version_info() {
    ...
    let result = Cli::try_parse_from(["txgate", "--version"]);
