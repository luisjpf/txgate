//! Test utilities for `TxGate` integration tests.
//!
...
/// Create a temporary `TxGate` installation for testing.
///
...
pub fn setup_test_env() -> TempDir {
    ...
    let txgate_dir = temp_dir.path().join(".txgate");
    let keys_dir = txgate_dir.join("keys");

    fs::create_dir_all(&keys_dir).expect("failed to create keys directory");

    // Create a minimal config.toml
    let config_content = r#"
# TxGate test configuration
[policy]
...
"#;
    ...
    fs::write(txgate_dir.join("config.toml"), config_content).expect("failed to write config.toml");
...
pub fn load_fixture(name: &str) -> Vec<u8> {
    // CARGO_MANIFEST_DIR points to crates/txgate, go up to workspace root
...
pub fn parse_ethereum_tx(raw: &[u8]) -> Result<ParsedTx, txgate_core::error::ParseError> {
