...
fn test_init_creates_directory_structure() {
    ...
    // Verify ~/.txgate directory is created
    assert!(txgate_dir.exists(), "~/.txgate directory should exist");
    assert!(txgate_dir.is_dir(), "~/.txgate should be a directory");
    ...
    let config_file = txgate_dir.join("config.toml");
...
fn test_parse_error_on_invalid_input() {
    ...
    assert!(
        matches!(err, txgate_core::signing::SigningError::ParseError(_)),
