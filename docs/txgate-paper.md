...
The name comes from Spanish: *txgate* means "seal" - every transaction gets your seal of approval, but only if policy allows.
...
TxGate is a single binary that:
...
```bash
curl -sSL https://txgate.dev/install.sh | sh
txgate init
txgate ethereum sign 0x02f8730181...
```
...
# Sign transaction
$ txgate ethereum sign 0x02f8730181948502540be400...
Parsed:
  Chain:     ethereum
  Type:      transfer
...
```bash
# Default location
~/.txgate/txgate.sock     # macOS/Linux
```
...
```bash
# Enable with flag
txgate serve --http 127.0.0.1:8080
```
...
│           │                                 │
│           ▼                                 │
│  Encrypted key file (~/.txgate/keys/*.enc)  │
└─────────────────────────────────────────────┘
```
